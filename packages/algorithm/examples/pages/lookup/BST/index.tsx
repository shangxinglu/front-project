...
export default defineComponent({
  setup(props) {
    ...
    const handleFindBtRank = () => {
        const rank = bst.rankNode(Number(formData.rank))
    ...
    const renderForm = () => {
      return (
        <div>
          ...
          <div class="mt-[15px] px-[15px]  flex items-center">
          <div class="w-[150px] text-right">排行：</div>
          <Input v-model:value={formData.rank} placeholder="请输入排行" />
          <Button
              type="primary"
              class="ml-[15px] w-[100px]"
              onClick={handleFindBtRank}
            >
              查找
            </Button>
          </div>
          <div class="mt-[15px] px-[15px]  flex items-center">
            ...
            <Input
              class="w-[200px]"
              placeholder="请输入更新key"
              v-model:value={formData.updateKey}
              type="text"
            />
            <Input
              class="ml-[15px]"
              placeholder="请输入更新值"
              v-model:value={formData.updateValue}
              type="number"
            />
            <Button
              type="primary"
